...
for(let i=0; i < inventory.length;i++){
    if(inventory[i].id === 33){
        let query = inventory[i]
console.log(`Car 33 is a ${query.car_year}, ${query.car_make}, ${query.car_model}` );
    }
}
// ==== Challenge 2 ====
// The dealer needs the information on the last car in their inventory.  What is the make and model of the last car in the inventory?  Log the make and model into the console.
let lastCar = inventory[inventory.length - 1];
console.log(`This vehicle is a ${lastCar.car_make} ${lastCar.car_model}`);
...
for(let i=0;i<carYears.length;i++){
    let currentCar = carYears[i]
    if(currentCar < 2000){
        oldCars.push(currentCar)
      //  console.log(`true for year of ${currentCar}`)
    }
}
...
for(let i=0;i<inventory.length;i++){
    let currentVehicle = inventory[i]
    if(currentVehicle.car_make === "Audi" || currentVehicle.car_make === "BMW"){
        BMWAndAudi.push(currentVehicle)
    }
}
